...
public class MelonService implements IMelonService {
     ...
     * 멜론 차트 수집 함수(웹 크롤링)
     */
    private List<MelonDTO> doCollecttest() throws Exception {
        String name = new Object() {}.getClass().getEnclosingMethod().getName();
        log.info(this.getClass().getName() + " /// "+ name+"/// Start!");
    ...
    private List<MelonDTO> doCollect() throws Exception {
        ...
        //int rank = 1;
        // Iterator을 사용하여 멜론차트 정보를 가져오기
        for (Element songInfo : element.select("tbody tr")) { //div.wrap_song_info

            // 크롤링을 통해 데이터 저장하기
            int rank = Integer.parseInt(CmmUtil.nvl(songInfo.select("div.wrap.t_center span.rank").eq(0).text())); // Rank
            String song = CmmUtil.nvl(songInfo.select("div.ellipsis.rank01 a").text()); // 노래
            String singer = CmmUtil.nvl(songInfo.select("div.ellipsis.rank02 a").eq(0).text()); // 가수
            ...
            // 가수와 노래 정보가 모두 수집되었다면, 저장함
            if ((song.length() > 0) && (singer.length() > 0) && (rank > 0)) {
